...
complete -c dotter -n "__fish_seen_subcommand_from gen-completions" -s s -l shell -d 'Set the shell for generating completions [values: bash, elvish, fish, powerShell, zsh]' -r
...
complete -c dotter -n "__fish_seen_subcommand_from help" -s g -l global-config -d 'Location of the global configuration' -r -F
complete -c dotter -n "__fish_seen_subcommand_from help" -s l -l local-config -d 'Location of the local configuration' -r -F
complete -c dotter -n "__fish_seen_subcommand_from help" -s d -l dry-run -d 'Dry run - don\'t do anything, only print information. Implies -v at least once'
complete -c dotter -n "__fish_seen_subcommand_from help" -s v -l verbose -d 'Verbosity level - specify up to 3 times to get more detailed output. Specifying at least once prints the differences between what was before and after Dotter\'s run'
complete -c dotter -n "__fish_seen_subcommand_from help" -s q -l quiet -d 'Quiet - only print errors'
complete -c dotter -n "__fish_seen_subcommand_from help" -s f -l force -d 'Force - instead of skipping, overwrite target files if their content is unexpected. Overrides --dry-run'
complete -c dotter -n "__fish_seen_subcommand_from help" -s y -l noconfirm -d 'Assume "yes" instead of prompting when removing empty directories'
complete -c dotter -n "__fish_seen_subcommand_from help" -s p -l patch -d 'Take standard input as an additional files/variables patch, added after evaluating `local.toml`. Assumes --noconfirm flag because all of stdin is taken as the patch'
