...
--         return require('structlog').get_logger('xvim')
--       end)
...
function Log:init()
      ...
      pipelines = {
        {
          ...
          sink = structlog.sinks.Console(true), -- async=false
        },
        {
  ...
  local logger = structlog.get_logger('xvim')
...
function Log:get_path()
  return string.format('%s/%s.log', vim.fn.stdpath('cache'), 'xvim')
end
